...
object OpaqueOperators extends Strategy {
  def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
    ...
    case EncryptedSort(sortOrder1, EncryptedSort(sortOrder2, EncryptedFilter(condition, child))) =>
      ObliviousFilterExec(condition, EncryptedSortExec(sortOrder2 ++ sortOrder1, planLater(child))) :: Nil
